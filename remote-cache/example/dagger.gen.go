...
func (r Example) MarshalJSON() ([]byte, error) {
	var concrete struct {
		Cache    *dagger.RemoteCache
		CacheKey string
	}
	concrete.Cache = r.Cache
	concrete.CacheKey = r.CacheKey
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Example":
		...
		case "CheckCache":
			...
			return (*Example).CheckCache(&parent, ctx)
		case "PrimeCache":
			var parent Example
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return nil, (*Example).PrimeCache(&parent, ctx, msg)
		case "":
			var parent Example
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			var cacheKey string
			if inputArgs["cacheKey"] != nil {
				err = json.Unmarshal([]byte(inputArgs["cacheKey"]), &cacheKey)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg cacheKey", err))
				}
			}
			return New(registry, repo, cacheKey), nil
		default:
