...
func (r *Local) UnmarshalJSON(bs []byte) error {
	var concrete struct{}
	err := json.Unmarshal(bs, &concrete)
	if err != nil {
		return err
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Local":
		...
		case "Export":
			...
			return nil, (*Local).Export(&parent, ctx, key, dir)
		case "Import":
			var parent Local
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Local).Import(&parent, ctx, key), nil
		default:
	...
	case "Registry":
		...
		case "Export":
			...
			return nil, (*Registry).Export(&parent, ctx, key, dir)
		case "Import":
			var parent Registry
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Registry).Import(&parent, ctx, key), nil
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "RemoteBackend":
		switch fnName {
		case "Local":
			var parent RemoteBackend
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
				panic(fmt.Errorf("%s: %w", "failed to unmarshal parent object", err))
			}
			return (*RemoteBackend).Local(&parent), nil
		case "Registry":
			var parent RemoteBackend
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*RemoteBackend).Registry(&parent, registry, repo, username, password), nil
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	default:
